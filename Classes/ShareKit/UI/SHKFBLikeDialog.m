...
@implementation SHKFBLikeDialog

@synthesize delegate = _delegate;
@synthesize webView = _webView;
@synthesize willHandleOrientationChanged;
...
- (void)updateWebOrientation {
	if (!willHandleOrientationChanged) return;
  UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
...
- (void)addObservers {
	if (willHandleOrientationChanged) {
...
- (id)init {
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _showingKeyboard = NO;
	  willHandleOrientationChanged = YES;  
    ...
    //UIImage* iconImage = [UIImage imageNamed:@"SocialMedia.bundle/twitter/twicon.png"];
    UIImage* closeImage = [UIImage imageNamed:@"SocialMedia.bundle/images/close.png"];
    
	  for (int x=0; x<4; x++) titleColor[x] = kFacebookBlue[x];
...
- (void)drawRect:(CGRect)rect {
  ...
  [self drawRect:headerRect fill:titleColor radius:0];
...
// resize webview inside so that field will not be blocked by keyboard
